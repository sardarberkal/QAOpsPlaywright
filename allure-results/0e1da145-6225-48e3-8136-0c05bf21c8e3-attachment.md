...
- Name: e2eFlowPO_Fixtures.spec.js >> @Web Client App Login for Adidas Orignals
...
```ts
  ...
  7  |         this.productTexts = page.locator(".card-body b");
  8  |         this.cart = page.locator("[routerlink*='cart']");
  9  |     }
  10 | 
  11 |    async searchProduct(productName){
  12 |             
> 13 |            await this.productTexts.first().waitFor();
     |                                            ^ Error: locator.waitFor: Test timeout of 30000ms exceeded.
  14 |            const titles = await this.productTexts.allTextContents();
  ...
  19 |                  //add to cart
  20 |                  await this.products.nth(i).locator("text= Add To Cart").click();
  21 |                  break;
  22 |               }
  23 |            }
  24 |     }
  25 | 
  26 |     async navigateToCart(){
